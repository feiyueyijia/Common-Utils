...
public class MultipleTreeUtils {
    ...
    private static String sort = BaseTree.NODE_LABEL;

    public static String getTreeList(List dataList, boolean flag, String orderBy) throws BusinessException {
        String result = "";
        isLeaf = flag;
        sort = orderBy;
        if (dataList != null && dataList.size() > 0) {
            ...
            for (Iterator it = dataList.iterator(); it.hasNext(); ) {
                ...
                Object extraProps = dataRecord.get("extraProps");

                if (level != null) {
                    if (StringUtils.isNumeric(level.toString())) {
                ...
                if (extraProps != null) {
                    node.extraProps = (List<ExtraProp>) extraProps;
                }
            ...
            for (Iterator it = entrySet.iterator(); it.hasNext(); ) {
                ...
                if (node.parentId == null || node.parentId.equals("")|| node.parentId.equals("-1")) {
                    ...
                    try {
                        ...
                        throw new BusinessException("sys.custom.error", "未检测到树形结构依据，节点： {" + node.id + " : " + node.label + "} 的父节点 " + node.parentId + " 不存在！");
                    }
                }
            }
            // 输出无序的树形菜单的JSON字符串
    ...
    static class Node {
         ...
         * 额外属性
         */
        private List<ExtraProp> extraProps = new ArrayList<>();
        ...
        public String toString() {
            ...
            if (extraProps != null && extraProps.size() > 0) {
                result += ", \"extraProps\" : [";
                for (ExtraProp prop : extraProps) {
                    result += "{\"" + prop.getPropName() + "\" : \"" + prop.getPropValue() + "\"}";
                    result += ",";
                }
    ...
    static class Children {
        ...
        public void sortChildren() {
            ...
            if (BaseTree.NODE_LEVEL.equals(sort)) {
                Collections.sort(list, new NodeIDComparator());
            } else {
                Collections.sort(list, new NodeLabelComparator());
            }
            // 对每个节点的下一层节点进行排序
            for (Iterator it = list.iterator(); it.hasNext(); ) {
...
//            int j2 = Integer.parseInt(((Node) o2).id);
     ...
     * 节点比较器
     */
    static class NodeLabelComparator implements Comparator {
        Collator cmp = Collator.getInstance(java.util.Locale.CHINA);

        public int compare(Object o1, Object o2) {
            String j1 = ((Node) o1).label;
            String j2 = ((Node) o2).label;
            if (cmp.compare(j1, j2) > 0) {
                return 1;
            } else if (cmp.compare(j1, j2) < 0) {
                return -1;
            }
            return 0;
        }
    ...
    public static void example() {
        ...
        List<ExtraProp> props = new ArrayList<>();
        ExtraProp prop1 = new ExtraProp();
        prop1.setPropName("name1");
        prop1.setPropValue("value1");
        ExtraProp prop2 = new ExtraProp();
        prop2.setPropName("name2");
        prop2.setPropValue("value2");
        props.add(prop1);
        props.add(prop2);
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            map.put("extraProps", props);
            list.add(map);
        }
        String treeJson = null;
        try {
            treeJson = MultipleTreeUtils.getTreeList(list, true, BaseTree.NODE_LABEL);
